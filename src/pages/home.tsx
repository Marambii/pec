...
export default function Home() {
  ...
  const pillars = [
    { label: "Person", question: "Who am I?", desc: "Unlocking raw structural identity by identifying the patterns of childhood without projecting blame onto parents, culture, or your environment." },
    { label: "Purpose", question: "Why am I here?", desc: "Discovering your core identity and unique alignment to unlock your life story as your greatest asset." },
    { label: "Process", question: "How do I grow?", desc: "Submitting to an intentional refining journey focused on emotional regulation protocols and step-by-step accountability." },
    { label: "Prize", question: "What impact will I make?", desc: "Reaching deep structural societal influence, systemic worth, and building a generational legacy that reaches far beyond yourself." }
  ];
  ...
  return (
    <div className="w-full space-y-24 animate-fadeIn">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center w-full">
        <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
          <span className="inline-block px-4 py-1.5 rounded-full bg-white shadow-xs text-xs font-bold tracking-widest text-[#1F4E79] uppercase border border-slate-100">
            ✨ Welcome to the Coaching Ecosystem
          </span>
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-[#1F4E79] leading-tight">
            Polishing Lives for <br />
            <span className="text-[#D4AF37] relative inline-block">Purpose & Excellence</span>
          </h1>
          <p className="text-base md:text-lg text-slate-600 max-w-2xl mx-auto lg:mx-0 font-light leading-relaxed">
            Helping individuals, families, and leaders discover deep identity, heal completely from structural life wounds, and step boldly into sustainable legacy.
          </p>
          <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-2">
            <button 
              onClick={() => navigate('/contactus')} 
              className="bg-[#1F4E79] text-white hover:bg-[#153654] px-8 py-3.5 rounded-full text-xs font-bold tracking-widest shadow-md transition-all uppercase"
            >
              Book a Session
            </button>
            <button 
        ...
        <div className="lg:col-span-5 relative flex justify-center">
          ...
          <div className="absolute top-12 -left-4 bg-white/95 backdrop-blur-md px-5 py-3 rounded-2xl shadow-md border border-slate-100 transform -rotate-2">
            <span className="text-[10px] font-bold text-[#D4AF37] uppercase block">Lifestyle</span>
            <span className="text-xs font-bold text-[#1F4E79]">Identity & Growth</span>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-5xl mx-auto w-full">
        {[
          { count: `${metrics.homes}+`, title: "Homes Restored", note: "Practical steps to rebuild trust." },
          { count: `${metrics.leaders}+`, title: "Leaders Aligned", note: "Refining people for significance." },
          { count: `${metrics.satisfaction}%`, title: "Restoration Rate", note: "True structural transformation." }
        ].map((metric, i) => (
      ...
      <section className="w-full">
        <SectionHeader 
          ...
          subtitle="The structure and teachings revolve around five pillars that create a complete transformational model."
        />
      ...
      <section className="bg-white rounded-[2rem] border border-slate-100 p-8 md:p-12 text-center max-w-5xl mx-auto w-full">
        <span className="text-[#1F4E79] uppercase tracking-widest text-xs font-bold block mb-2">
          Daily Transformative Protocol
        </span>
        <h2 className="text-2xl md:text-3xl font-serif font-bold text-[#1F4E79] mb-10">
          Our Emotional Regulation Methodology
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { step: 'STOP', act: 'Pause immediately before reacting blindly.' },
            { step: 'THINK', act: 'Evaluate honestly what you are feeling beneath the wound.' },
            { step: 'RESPOND', act: 'Speak calmly, transparently, and constructively.' }
          ].map((item, idx) => (
      ...
      <section className="w-full">
        <SectionHeader 
          tag="Tailored Frameworks" 
          title="Signature Ecosystem Programs" 
          subtitle="Deliberate tracks built for complete transformational alignment and identity discovery." 
        />
        ...
        <div className="bg-white rounded-[2rem] p-8 md:p-12 border border-slate-100 shadow-xs relative text-center">
          ...
          <div className="flex items-center justify-center space-x-3">
            ...
            <div className="text-left">
              <h4 className="text-xs font-bold text-[#1F4E79]">{TESTIMONIALS_DATA[0].name}</h4>
              <p className="text-[10px] text-slate-400 font-light">{TESTIMONIALS_DATA[0].role} &bull; {TESTIMONIALS_DATA[0].location}</p>
            </div>
          </div>
        </div>
      </section>

      {/* 7. REUSABLE CALL TO ACTION INTERFACE */}
      <CallToActionBanner onAction={() => navigate('/contactus')} />
